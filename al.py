...
while True:
    ...
    #
        ...
        if(wait_prod == False and productor.time > 0): #Si aun tiene tiempo y puede trabajar o dormir
            ...
            #wait_cons = False
            if(productor.estado == "Trabajando"):#Si el estado era trabajando apagamos semaforo
                semaforo = False
        ...
        if(semaforo == False):
            ...
            if(productor.estado == "Esperando"):#Si esta esperando tiene prioridad para entrar
                if(checkProdAvailable()):
                    ...
                    wait_prod = False
                    #wait_cons = True
                    if(consumidor.estado_exp != "!"):#Cuando se esta realizando un proceso se asigna este caracter
                        #Se esta esperando a asignarse un trabajo
                        if(consumidor.estado_exp == "Trabajando" and checkConsAvailable()):
                            #Si el consumidor quiere trabajar y puede trabajar
                            if(semaforo == False):#El productor no pudo entrar
                                #Se pone a trabajar el consumidor
                                consumidor.estado = "Trabajando"
                                consumidor.estado_exp = "!"
                                semaforo = True
                                wait_cons = False
                            else:
                                #Estado esperando True el wait_cons
                                consumidor.estado = "Esperando"
                                consumidor.estado_exp = "!"
                                wait_cons = True
                            consumidor.time = random.randrange(3, 11, 1)#Asignacion de tiempo
                        elif(consumidor.estado_exp == "Durmiendo"):#Para cuando el consumidor quiera dormir
                            consumidor.estado_exp = "!"#Caracter de control
                            consumidor.estado = "Durmiendo"
                            consumidor.time = random.randrange(3, 11, 1)
                elif(checkProdAvailable() == False):#En caso de que el productor no pueda trabajar
                ...
                if(checkConsAvailable()):
                    #Analizamos al consumidor
                    consumidor.estado = "Trabajando"
                    semaforo = True
                    wait_cons = False
                    if(productor.estado_exp != "!"):
                        #Trabajamos con el productor
            ...
            elif(productor.estado_exp == consumidor.estado_exp == "Trabajando"):#Verificamos
        ...
        elif(semaforo == True):#Semaforo prendido o hay alguien trabajando
            #Uno trabajando, el otro quiere trabajar
            #Uno trabajando, el otro quiere dormir
            if(productor.estado_exp == "Trabajando"): #Poner a esperar, si el productor quiere trabajar
                productor.estado_exp = "!"
                productor.estado = "Esperando"
                wait_prod = True
                productor.time = random.randrange(3, 11, 1)
            elif(productor.estado_exp == "Durmiendo"): #Si quiere dormir se pone durmiendo
        ...
        if(productor.estado == "Trabajando" and checkProdAvailable() == True):
            producir(productor.pos)#Mandamos la posicion de nuestro productor
            if(productor.pos < 29):
                productor.pos += 1
            else:
                productor.pos = 0
